...
/****************************************************************************/
  ...
  inline void RF24::beginTransaction() {
	...
	#endif
	PORTB &= ~_BV(PORTB2); // CSN pin 10: LOW
    _delay_us(5);
  }
...
/****************************************************************************/
  ...
  inline void RF24::endTransaction() {
	PORTB |= _BV(PORTB2); // CSN pin 10: HIGH
    _delay_us(5);
	#if defined (RF24_SPI_TRANSACTIONS)
    SPI.endTransaction();
	#endif
...
RF24::RF24():
  p_variant(false), payload_size(32), dynamic_payloads_enabled(false), addr_width(5) //,pipe0_reading_address(0)
{
...
RF24::RF24(uint32_t _spi_speed):
  spi_speed(_spi_speed), p_variant(false), payload_size(32), dynamic_payloads_enabled(false), addr_width(5) //,pipe0_reading_address(0)
{
  pipe0_reading_address[0]=0;
}
...
void RF24::stopListening(void)
{
  ...
  for (volatile uint32_t i = 0; i < txRxDelay; i++) _delay_us(1);
...
{
  PORTB &= ~_BV(PORTB1); // CE pin 9: Guarantee CE is low on powerDown
...
void RF24::errNotify(){
	...
	failureDetected = 1;
	#else
	for (volatile uint16_t i = 0; i < 5000; i++) _delay_ms(1);
	#endif
}
#endif
/******************************************************************/
...
bool RF24::write( const void* buf, uint8_t len, const bool multicast )
{
	...
	#if defined (FAILURE_HANDLING)
		uint32_t timer = sys_time.millis();
	#endif

	while( ! ( get_status()  & ( _BV(TX_DS) | _BV(MAX_RT) ))) { 
		#if defined (FAILURE_HANDLING)
			if(sys_time.millis() - timer > 95){
				errNotify();
				#if defined (FAILURE_HANDLING)
				  return 0;
				#else
				  for (volatile uint8_t i = 0; i < 100; i++) _delay_ms(1);
				#endif
			}
		#endif
	}
...
bool RF24::writeBlocking( const void* buf, uint8_t len, uint32_t timeout )
{
	...
	uint32_t timer = sys_time.millis();							  //Get the time that the payload transmission started
	...
	while( ( get_status()  & ( _BV(TX_FULL) ))) {		  //Blocking only if FIFO is full. This will loop and block until TX is successful or timeout
		...
		if( get_status() & _BV(MAX_RT)){					  //If MAX Retries have been reached
			...
			if(sys_time.millis() - timer > timeout){ return 0; }		  //If this payload has exceeded the user-defined timeout, exit and return 0
		}
		#if defined (FAILURE_HANDLING)
			if(sys_time.millis() - timer > (timeout+95) ){
				errNotify();
				#if defined (FAILURE_HANDLING)
				return 0;
                #endif
...
void RF24::reUseTX(){
		...
		PORTB &= ~_BV(PORTB1); // CE pin 9: Re-Transfer packet
		PORTB |= _BV(PORTB1);
}
...
void RF24::startFastWrite( const void* buf, uint8_t len, const bool multicast, bool startTx){ //TMRh20
	...
	if(startTx) PORTB |= _BV(PORTB1); // CE pin 9
...
bool RF24::txStandBy(){
    ...
    #if defined (FAILURE_HANDLING)
		uint32_t timeout = sys_time.millis();
	#endif
	...
	PORTB &= ~_BV(PORTB1); // CE pin 9: Set STANDBY-I mode
	return 1;
}
...
bool RF24::txStandBy(uint32_t timeout, bool startTx){
	...
	uint32_t start = sys_time.millis();

	while( ! (read_register(FIFO_STATUS) & _BV(TX_EMPTY)) ){
		if( get_status() & _BV(MAX_RT)){
			write_register(NRF_STATUS,_BV(MAX_RT) );
				PORTB &= ~_BV(PORTB1); // CE pin 9: Set re-transmit
				PORTB |= _BV(PORTB1);
				if(sys_time.millis() - start >= timeout){
					PORTB &= ~_BV(PORTB1); // CE pin 9
					flush_tx();
					return 0;
				}
		...
		#if defined (FAILURE_HANDLING)
			if( sys_time.millis() - start > (timeout+95)){
				errNotify();
				#if defined (FAILURE_HANDLING)
				return 0;	
				#endif
...
bool RF24::setDataRate(rf24_datarate_e speed)
{
  ...
  txRxDelay=85; //16MHz Arduino
  if( speed == RF24_250KBPS )
  {
    // Must set the RF_DR_LOW to 1; RF_DR_HIGH (used to be RF_DR) is already 0
    // Making it '10'.
    setup |= _BV( RF_DR_LOW ) ;
	txRxDelay=155; //16MHz Arduino
  }
  else
  {
